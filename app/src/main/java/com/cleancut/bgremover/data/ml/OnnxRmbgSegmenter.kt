...
/**
 * On-device SOTA matting using Bria AI RMBG-1.4 via ONNX Runtime Mobile.
 * Provides studio-level edge and hair accuracy on a 1024x1024 input tensor.
 */
    ...
    suspend fun segment(originalBitmap: Bitmap): Result<SegmentationResult> = withContext(Dispatchers.Default) {
        try {
            ...
            execTime = measureTimeMillis {
                ...
                // 2. Normalize and prepare NCHW FloatBuffer
                ...
                for (i in 0 until planeSize) {
                    ...
                    floatBuffer.put(i, (r - 0.5f) / 0.5f)
                }
                // Channel G
                for (i in 0 until planeSize) {
                    val g = (inputPixels[i] shr 8 and 0xFF) / 255.0f
                    floatBuffer.put(planeSize + i, (g - 0.5f) / 0.5f)
                }
                // Channel B
                for (i in 0 until planeSize) {
                    val b = (inputPixels[i] and 0xFF) / 255.0f
                    floatBuffer.put(2 * planeSize + i, (b - 0.5f) / 0.5f)
                }
                floatBuffer.rewind()

                // 3. Create input tensor and execute session
                ...
                // 5. Bilinear upsample mask1024 directly to original high-res dimensions
